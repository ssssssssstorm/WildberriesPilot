...
def shift_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="Сбросить демо", callback_data="reset")],
        ]
    )
...
def task_keyboard(task: Task) -> InlineKeyboardMarkup:
    ...
    if task.requires_confirmation:
        ...
        rows.append([InlineKeyboardButton(text="Подтвердить и отправить", callback_data=f"send:{task.id}")])
    rows.extend(
        [
            [InlineKeyboardButton(text="Редактировать черновик", callback_data=f"edit:{task.id}")],
            [
